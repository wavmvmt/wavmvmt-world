...
export function HUD() {
  ...
  const [showPanel, setShowPanel] = useState<'none' | 'build' | 'fund'>('none')
  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-10" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      {isMobile && (
        <div className="absolute top-14 right-3 flex gap-2 pointer-events-auto">
          <button
            onClick={() => setShowPanel(showPanel === 'build' ? 'none' : 'build')}
            className="px-3 py-1.5 rounded-xl text-[0.6rem] tracking-wider uppercase"
            style={{
              ...panelStyle,
              color: showPanel === 'build' ? '#f0c674' : 'rgba(255,220,180,0.4)',
              cursor: 'pointer',
            }}
          >
            Build
          ...
          <button
            onClick={() => setShowPanel(showPanel === 'fund' ? 'none' : 'fund')}
            className="px-3 py-1.5 rounded-xl text-[0.6rem] tracking-wider uppercase"
            style={{
              ...panelStyle,
              color: showPanel === 'fund' ? '#f0c674' : 'rgba(255,220,180,0.4)',
              cursor: 'pointer',
            }}
          >
            Fund
      ...
      {(!isMobile || showPanel === 'build') && (
        <div
          className={`absolute p-3 md:p-3.5 rounded-2xl min-w-[180px] md:min-w-[200px] max-h-[45vh] overflow-y-auto pointer-events-auto ${
            isMobile ? 'top-24 right-3 left-3' : 'top-5 right-5'
          }`}
          style={panelStyle}
        >
          <div className="text-[0.55rem] md:text-[0.6rem] tracking-[0.25em] uppercase mb-2 md:mb-2.5" style={{ color: 'rgba(255,220,180,0.4)' }}>
      ...
      {(!isMobile || showPanel === 'fund') && (
        <div
          className={`absolute p-3 md:p-3.5 rounded-2xl min-w-[200px] md:min-w-[210px] max-h-[60vh] overflow-y-auto pointer-events-auto ${
            isMobile ? 'top-24 right-3 left-3' : 'top-5 left-5'
          }`}
          style={panelStyle}
        >
          <div className="text-[0.55rem] md:text-[0.6rem] tracking-[0.25em] uppercase mb-2 md:mb-3" style={{ color: 'rgba(255,220,180,0.4)' }}>
            Real-World Build
          </div>
...
function MobileControls() {
  useEffect(() => {
    // Dispatch keyboard events from touch controls
  ...
  return (
    <>
      {/* Right side — look area (transparent, takes up right half) */}
      <div
        id="mobile-look"
